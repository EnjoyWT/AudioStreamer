...
//        }
//        
        let result = data.withUnsafeBytes { (bytes: UnsafeRawBufferPointer) -> OSStatus in
            let buffer = bytes.bindMemory(to: UInt8.self)
            return AudioFileStreamParseBytes(streamID, UInt32(data.count), buffer.baseAddress, [])
        }
